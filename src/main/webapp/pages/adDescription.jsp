...
<pre>
	<a href="/">Home</a> &gt; <a href="/profile/myHouses">My Houses</a> &gt; Ad Description</pre>
...
<script>
	...
	function initMap() {
		...
		var address = document.getElementById("adAddress").innerHTML;
		var title = document.getElementById("shownAdTitle").innerHTML;
	...
	$(document).ready(function() {
		$('a[href="#ad-map"]').on('click', function() {
			setTimeout(function() {
...
<section>
				...
				<c:choose>
					<c:when test="${shownAd.user.isPremium()}">
						<a href="/ad/placeOnHomepage?id=${shownAd.id}">
							<button type="button" class="btn btn-default">Place on
								homepage</button>
						</a>
					</c:when>
				</c:choose>
	...
	<div class="container-fluid">
		<div class="row">
			<div class="col-md-6">
				<table class="table">
					...
					<tr>
						...
						<td><c:choose>
								<c:when test="${shownAd.isOnHomepage()}">
									<img src="/img/check-mark.png" width="26px">
								</c:when>
								<c:otherwise>
									<img src="/img/check-mark-negative.png" width="26px">
								</c:otherwise>
							</c:choose></td>
					</tr>
				</table>
			...
			<div class="col-md-6 ad-tabs" id="tabs">
				<ul>
					<li><a href="#carousel-example-generic">Pictures</a></li>
					<li><a href="#ad-map">Map</a></li>
				</ul>
					...
					<c:when test="${shownAd.isAuction()}">
						...
						<table class="table">
							...
							<c:choose>
								<c:when test="${loggedIn}">
									<form:form method="post" modelAttribute="betForm"
										...
										<tr>
											<td><span class="auctionUsername">
													${loggedInUserEmail} </span></td>
											<td><form:input id="field-Price" type="number"
													path="price" placeholder="Bid"
													step="${shownAd.getAuctionStepPrice()}"
													class="form-control"
													value="${shownAd.getHighestBet()+shownAd.getAuctionStepPrice()}"
													min="${shownAd.getHighestBet()+shownAd.getAuctionStepPrice()}" />
							...
							<c:forEach items="${shownAd.bets}" var="bet" varStatus="status">
								...
								<fmt:formatNumber value="${bet.price}" type="currency"
									currencySymbol="" var="formattedPrice" />
								<fmt:formatDate value="${bet.creationDate}"
									var="formattedBetDate" type="date" pattern="dd.MM.yyyy" />
								<tr>
				...
				<table id="advertiserTable" class="table">
					<tr>
						...
						<td id="advertiserEmail"><c:choose>
								<c:when test="${loggedIn}">
									<a href="/user?id=${shownAd.user.id}"><button
											class="btn btn-default" type="button">Visit profile</button></a>
								</c:when>
								<c:otherwise>
						...
						<td>
							<form>
								<c:choose>
									<c:when test="${loggedIn}">
										<c:if test="${loggedInUserEmail != shownAd.user.username }">
											<button id="newMsg" class="btn btn-default" type="button">Contact
												Advertiser</button>
										</c:if>
									</c:when>
									<c:otherwise>
						...
						<c:forEach items="${visits }" var="visit">
							<tr>
								...
								<td><c:choose>
										<c:when test="${loggedIn}">
											<c:if test="${loggedInUserEmail != shownAd.user.username}">
												<button class="btn btn-default thinButton" type="button"
													data-id="${visit.id}">Send enquiry to advertiser</button>
											</c:if>
										</c:when>
										<c:otherwise>
											<a href="/login"><button
													class="btn btn-default thinInactiveButton"
													disabled="disabled" type="button" data-id="${visit.id}">Login
													to send enquiries</button></a>
										</c:otherwise>
									</c:choose></td>
							</tr>
						</c:forEach>
...
<div id="msgDiv">
	<form class="msgForm">
		...
		<br> <br> <label>Subject: <span>*</span></label> <input
			class="msgInput form-control" type="text" id="msgSubject"
			placeholder="Subject" /> <br> <br> <label>Message:
		</label>
...
<div id="confirmationDialog">
	<form>
		...
		<button type="button" class="btn btn-default"
			id="confirmationDialogSend">Send</button>
		<button type="button" class="btn btn-default"
			id="confirmationDialogCancel">Cancel</button>
	</form>
</div>

<script async defer
	src="https://maps.googleapis.com/maps/api/js?key=AIzaSyB5Hs4TYmXUiMKPetPokSZrUSMnev58Fm8&callback=initMap"
